...
- **`topK` is a tuning gate, not a free constant.** It is the pre-rerank grounding pool; the **Needle-Scatter** eval (§6) requires that pool to hold ≥95% of dispersed relevant items. `scatterRecall(grounding, needles)` is the check; the 8-Needle CI **seed** exercises it today, and the full 8-needle `assert(scatterRecall ≥ 0.95)` gate (+ CI un-freeze) is pending — once it lands, `topK` can never silently fall below the bar (raise it or add a recall-floor if it drops needles).

**Honest status (Forge audit — own it).** These are the *target* guardrails, not live enforcement yet; the footer's "blueprint, not built" applies to §4 too:
- **Not wired into the live path.** `retrieveGrounding()` / `scatterRecall()` / `int8Dot` exist and pass local unit tests @ `c58c0df` (now on origin via the gateway lane), but `/answer` + `/brief` still ground on the whole verified bundle — correct at one-checkpoint scale. The relevance contract goes live only when the memory-scale `/answer` calls `retrieveGrounding()` instead of mapping the full bundle; that WIRE is the pending close, not the functions.
- **CI is not asserting it yet.** Actions are frozen (billing), and the current scatter check is ONE synthetic case, not the full ENGRAM §11 8-Needle harness (500 planted chains over real corpora). "≥0.95 in CI" is the bar to meet, not a passing measurement.
- **Relevance ≠ faithfulness (named open guardrail).** "Grounded" here means *cited an id in the RRF-relevant top-K* — the **relevance** axis. It does **not** yet mean the answer prose is **entailed by** the cited item — the **faithfulness** axis: a confident-wrong answer that cites a relevant needle its text doesn't actually follow from still routes `.answered`. Entailment/faithfulness is the harder half of grounding-first honesty at memory scale, and it is an **open** guardrail, not one §4 enforces today.
