...
- **Track A (now, hermetic):**
  ...
  ```
  Proves ring → answer → same-session governed write, **real ed25519**, the invariant `ring.sessionId === writeback.sessionId`, idempotent (no double-post), non-member → 403. Plus the iOS **simulator** renders the "Senti is calling" UI and the full DIALS flow (capture via a side-loaded Whisper model). Narrative: the demo-guide.
- **Track B (infra, for a live phone):** APNs VoIP cert · api#753 Deploy-to-ECS (writeback endpoint live) · Apple code-signing (app on your real phone) · one-time Whisper side-load (`<AppSupport>/PocketModels/ggml-base.en.bin`, sha256 `a03779…`) · the deploy `apnsSend` delivers `buildVoipPushDictionary(payload)` top-level.
