...
Xcode invocation.
The empty `OTHER_SWIFT_FLAGS` policy does not independently authenticate the selected Xcode installation, Swift
driver executable, process environment, or final frontend argv; those remain workflow/toolchain evidence rather than
claims made from the settings JSON alone.
