...
- Relay (gateway/clients):
  ...
  - **APP FIXTURE ALREADY SIGNED — supersedes the FORGE-DAY signing under Atlas above.** I signed `canonical_checkpoint.json` today with a fresh non-forgeable key (Node ed25519). Just patch 2 fields + add the pubkey: `signingKeyId="pocket-demo-app-fixture"`, `signature="ZdyelUEuYxRg0SpGaefRj8Qq2lK_r_SlDex5b4TqWZH1M_Yi2KUh5cT_tyiaHDM2HcoYjPZwgZIhKO5BmF-YBg"`, `pocketTrustedGatewayKeys += {"pocket-demo-app-fixture":"SehNmI_dP9XFonEUXzmoDA7B0wCAss_JbVbbM4L0Y94"}`. BYTE-VERIFY FIRST: Swift `canonicalBytesUtf8` over the decoded fixture sha256 == `51d9db4d56dc1ee49e9ca1fbb6825aef3bc5951259891885568b184fcc0053d8`. Re-sign after any fixture edit: `node services/pocket-gateway/scripts/sign-app-fixture.mjs`.
  - **F2 budget single-source**: pin `PocketBundle.maxTotalElements/maxTotalBytes` to **20000 / 1048576** (`BUNDLE_BUDGET` in `src/bundle.mjs`). Every gateway sign path enforces the 512KiB ceiling (`MAX_BUNDLE_BYTES=524288`) < 1MiB phone budget → egress ⊆ phone acceptance by the 2× margin. Per-field caps == `SUMMARY_CAPS` (256/128/512/8192/8000/…).
