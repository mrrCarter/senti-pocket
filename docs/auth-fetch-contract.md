...
- **Ownership:** no `SessionRow`/`CheckpointContent` here (PR16/Atlas domain); the factory consumes these wire pages.
...
- **Server-lane P1:** preserve validated OAuth token kind+scope on `AuthenticatedUser` (stop discarding), enforce `sessions:read/write/usage` by route, verify iss/aud/sig/exp, negative read-token→write=403 KAV, deploy + live probe. (Owner TBD — relay offered.)
- **Pocket client registration:** add a Pocket `client_id`+callback to `MCP_OAUTH_CLIENTS`.
Until BOTH land, Pocket auth is fixture-only; the `.fixture` transport claims no live authorization.
