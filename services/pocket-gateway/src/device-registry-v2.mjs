...
export function validateDeviceUnregistrationV2(body) {
  const shape = exactObject(
    body,
    new Set(['registrationVersion', 'installationId', 'sessionId', 'bindingId', 'bindingRevision']),
  );
  if (shape) return shape;
  ...
  return {
    ok: true,
    value: { registrationVersion, installationId, sessionId, bindingId, bindingRevision },
  };
}
...
function registrationFields({ hmacKey, principal, humanId, value, nowEpochSec, leaseSeconds }) {
  ...
  const requestFingerprint = registrationFingerprint(hmacKey, targetKey, voipTokenHash, value.platform);
  ...
  return { installationKey, targetKey, tokenKey, voipTokenHash, requestFingerprint, expiresAtEpochSec };
}

function reclaimProtectedTokenClaim(claim, tokenKey, nowEpochSec) {
  if (!claim) return null;
  assertStoredTokenClaim(claim, tokenKey);
  return claim.ttl > nowEpochSec ? claim : null;
}

function authorizeTokenClaim({ claim, tokenKey, currentBinding, value, nowEpochSec }) {
  const protectedClaim = reclaimProtectedTokenClaim(claim, tokenKey, nowEpochSec);
...
export function createInMemoryDeviceRegistryV2({
  ...
  return {
    ...
    async register({ principal, humanId, ...body } = {}) {
      ...
      if (current) assertStoredBinding(current, f.installationKey);
      ...
      if (current && current.requestFingerprint === f.requestFingerprint) {
        ...
        resultOptions = {
          idempotent: current.idempotencyKey === value.idempotencyKey,
      ...
      if (current && (current.platform !== value.platform || current.voipTokenHash !== f.voipTokenHash)) {
        const oldTokenKey = deriveDeviceTokenKey(key, current.platform, current.voipToken);
        const oldClaim = tokenClaims.get(oldTokenKey);
        if (oldClaim) {
          assertStoredTokenClaim(oldClaim, oldTokenKey);
          if (
            oldClaim.ownerInstallationKey === current.pk &&
            oldClaim.bindingId === current.bindingId &&
            oldClaim.bindingRevision === current.bindingRevision
          ) {
    ...
    async unregister({ principal, humanId, ...body } = {}) {
      ...
      return { removed: Boolean(matches) };
    },
...
export function createDynamoDeviceRegistryV2({
  ...
  const basePut = (nextBinding, current) => ({
    Put: {
      ...
      ConditionExpression: current
        ? [
            '#schema = :schema AND #bindingId = :bindingId AND #revision = :revision',
  ...
  const baseDelete = (binding) => ({
    Delete: {
      ...
      ConditionExpression: [
        '#schema = :schema AND #target = :target',
  ...
  const claimPut = (nextClaim, protectedClaim, nowEpochSec) => ({
    Put: {
      ...
      ConditionExpression: protectedClaim
        ? '#schema = :schema AND #claimId = :claimId AND #claimRevision = :claimRevision'
  ...
  return {
    ...
    async register({ principal, humanId, ...body } = {}) {
      ...
      for (let attempt = 0; attempt < DEVICE_REGISTRY_MAX_SERIALIZATION_ATTEMPTS; attempt += 1) {
        ...
        try {
          const f = registrationFields({ hmacKey: key, principal, humanId, value, nowEpochSec, leaseSeconds });
          const current = await observe(observations, f.installationKey, BINDING_SK);
          if (current) assertStoredBinding(current, f.installationKey);
          ...
          let oldTokenKey;
          if (current && (current.platform !== value.platform || current.voipTokenHash !== f.voipTokenHash)) {
            oldTokenKey = deriveDeviceTokenKey(key, current.platform, current.voipToken);
      ...
      for (let attempt = 0; attempt < DEVICE_REGISTRY_MAX_SERIALIZATION_ATTEMPTS; attempt += 1) {
        ...
        try {
          ...
          const matches =
            ...
            current.bindingId === value.bindingId &&
            current.bindingRevision === value.bindingRevision;
          if (!matches) return { removed: false };
