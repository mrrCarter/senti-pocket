data "aws_signer_signing_profile" "gateway" {
  count = local.production_gateway_enabled ? 1 : 0

  name = var.gateway_signing_profile_name

  lifecycle {
    postcondition {
      condition     = self.platform_id == "AWSLambda-SHA384-ECDSA"
      error_message = "The production gateway signing profile must use AWSLambda-SHA384-ECDSA."
    }

    postcondition {
      condition     = self.status == "Active"
      error_message = "The production gateway signing profile must be Active."
    }
    ...
    postcondition {
      condition = self.arn == join("", [
        ...
        var.gateway_signing_profile_name,
      ])
...
resource "aws_lambda_code_signing_config" "gateway" {
  count = local.production_gateway_enabled ? 1 : 0

  description = "Senti Pocket production gateway trust policy"
  ...
  lifecycle {
    postcondition {
      condition = (
        ...
        toset([data.aws_signer_signing_profile.gateway[0].version_arn])
      )
      error_message = "The gateway code-signing configuration must trust exactly one immutable profile version."
    }
    ...
    postcondition {
      ...
      error_message = "Untrusted gateway Lambda artifacts must be rejected."
    }
  }
