...
- **Registry V2 items** (required only for `DEVICE_REGISTRY_MODE=v2`; no GSI):
  ...
  - Registration, rebind, token-owner transfer, renewal, and revoke update every affected base/claim/directory in one
    `TransactWrite`. Lookup strongly reads one directory and then at most 20 exact base/claim pairs. An explicit token
...
an old route as active after a faster worker has automatically reassigned its token. Cross-owner transfer during the
grace is still available through the exact token-claim CAS and removes the displaced state transactionally.
...
The 300-second grace is persisted-schema behavior and deployment-immutable for Registry V2. This code atom assumes the
V2 namespace has never been activated: verify all `dial:install:v2:`, `dial:token:v2:`, and `dial:target:v2:` prefixes
are empty before its first enablement. If any prerelease V2 worker or row exists, disable V2 traffic, purge those rows,
deploy the homogeneous fleet, force cold starts, and only then enable registration/ringing. Never roll this atom beside
a worker that writes `ttl = expiresAtEpochSec`; a future grace change requires a versioned namespace migration.
...
| `DEVICE_REGISTRY_MODE=v2` + `DEVICE_REGISTRY_HMAC_SECRET_ARN` + `DEVICE_REGISTRY_V1_PURGED=1` + `DEVICE_REGISTRY_CLIENT_V2_READY=1` | installation-owned Registry V2; the reference handler resolves the secret to the process-local `DEVICE_REGISTRY_HMAC_KEY_B64` expected by the gateway |
...
transformed into installation-owned rows. Before setting `DEVICE_REGISTRY_V1_PURGED=1`, disable V1 registration,
delete every legacy row, verify the prefix is empty, and only then flip `DEVICE_REGISTRY_MODE=v2`.
...
against V2. Both acknowledgements are boot gates, not automatic proof—the release/purge checks remain operator-owned.
...
ring dispatch, purge every `dial:install:v2:`, `dial:token:v2:`, and `dial:target:v2:` row under the old key, replace
...
- With a valid senti session bearer: `GET /sync`, `GET /checkpoint?sessionId=…`, `POST /answer`, `POST /brief`,
  `POST /actions/execute`, `POST /tts`, `POST /deck`, `POST /dial`, and `POST|DELETE /dial/register` per the granted scopes.
- In V2 mode, prove same-installation A→B movement: A stops resolving, B receives a `v:2` push with its exact nested
  binding tuple, stale A unregister is a no-op, and a logically expired binding is hidden before Dynamo TTL cleanup.
