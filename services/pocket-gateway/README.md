...
- **Auth (done):** `src/auth.mjs` — REAL AIdenID bearer verification: JWT signature against a JWKS, `iss/aud/exp/nbf`, and DPoP proof-of-possession (RFC 9449 — thumbprint binding + `htm/htu/ath/iat`). Not a stub; the JWKS/issuer/audience are deploy config.
- **Store (done):** `src/store.mjs` — async store: in-memory (dev/tests) + `createDynamoStore` (real conditional-put lock + `putIfAbsent` + TTL; deploy injects the `@aws-sdk/lib-dynamodb` client → package stays zero-dep).
