...
describe("RoomAdmissionShard invariants", () => {
  ...
  it("rejects principal and provider identity aliasing inside one shard", async () => {
    await prime();
    await createActive();
    const secondKey = mutateWithinShard(participantKey);
    ...
    ).toEqual({ disposition: "principal_conflict" });
  ...
  it("partitions the room admission budget exactly and never grows past a shard limit", async () => {
    ...
    const first = await shard.reserveAdmission(
      reservation({
        maxDailyAdmissionsPerRoom: exactRoomMaximum,
      }),
    );
    if (first.disposition !== "create") {
...
function mutateWithinShard(participantKey: string): string {
  const replacement = participantKey.endsWith("A") ? "B" : "A";
  return `${participantKey.slice(0, -1)}${replacement}`;
}
